...
class ProductController extends Controller
{
    ...
    public function store(ProductRequest $request)
    {
        ...
        if($request->hasFile('extra1')) 
            $data['extra1'] = $request->file('extra1')->store('images/product', 'public');

        if($request->hasFile('extra2')) 
            $data['extra2'] = $request->file('extra2')->store('images/product', 'public');
    ...
    {        
        ...
        if($request->hasFile('extra2')){
            if (Storage::disk('public')->exists($product->extra2))
                    Storage::disk('public')->delete($product->extra2);
